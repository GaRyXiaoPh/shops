...
public class QcloudCloudStorageService extends CloudStorageService {
	...
	public String upload(InputStream inputStream, String path) {
		...
		try {
			...
			throw new ServerException("文件流错误，" + e1.getMessage());
		...
		try {
			...
			throw new ServerException("服务异常，" + e.getErrorMessage());

		} catch (CosClientException e) {

			throw new ServerException("客户端异常，" + e.getMessage());
		} catch (InterruptedException e) {

			throw new ServerException("系统异常，" + e.getMessage());
		} finally {
			// 关闭 TransferManger
