...
public interface AdMapper {
     ...
     * @return
     */
    List<AdResVO> queryADList(@Param("locale") String locale,@Param("position") String position);


    Page<AdResVO> getAdResVOListPage(@Param("status") Integer status,@Param("position") String position,RowBounds rowBounds);

    @Select(" select * from t_advertise WHERE  position = #{position}")
    AdResVO getAdResVOByPosition(@Param("position") String position);
}
