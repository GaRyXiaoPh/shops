...
import com.cmd.wallet.blockchain.bitcoin.BitcoinJSONRPCClient;
import com.cmd.wallet.blockchain.bitcoin.BitcoinRException;
...
public class BitcoinService {
    ...
    public int syncNewTransactions(String coinName) throws MalformedURLException {
        ...
        while(curBlock < toBlock) {
            try {
                syncBlock(coin, client, curBlock, blockNum);
                curBlock++;
                count++;
                if(count >= 1000) {
     ...
     */
    private void syncBlock(Coin coin, BitcoinJSONRPCClient client, int blockHeight, int maxBlockNum) {
    ...
    private void syncTransaction(Coin coin, BitcoinJSONRPCClient client, String transactionHash) {
        ...
        //    return;
        int status = 1;
        int confirm = Integer.parseInt(mTrans.get("confirmations").toString());
        if(confirm < txConfirmCount) {
            ...
            if(status < -99) {
                log.warn("syncTransaction status==" + status + ",confirmCount=" + confirm + ",ConfirmCount "
                        + txConfirmCount + ",mTrans=" + ReflectionToStringBuilder.toString(mTrans));
                status = -99;
            }
        }
        ...
        for (int i=0; i<details.size(); i++){
            ...
            log.info("find valid transaction:" + transactionHash + ", content=" + ReflectionToStringBuilder.toString(mTrans) + ",status=" + status);
            ...
            receivedCoinService.addTransaction(userId, coin.getName(), address, transactionHash, recv, fee, time, 1, address);
        }
    }

    // 同步老的交易
    private void syncOldTransaction(Coin syncCoin, BitcoinJSONRPCClient client) {
        ...
        for(String txId : oldTransactions) {
            log.info("begin syn old transaction " + txId);
            try {
